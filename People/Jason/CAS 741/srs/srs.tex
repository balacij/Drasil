...
\begin{figure}[h!]
    \begin{center}
        \begin{tikzpicture}
            ...
            \draw [] (E) -- (BB) node[midway,right] {\parbox{2.5cm}{\tiny\center{}BVP Solver receives BVP specifications and responds with solution or error.}};
        \end{tikzpicture}
    \end{center}
...
\Cref{beam_bending_diagram} shows the physical system at rest. A slender beam is
...
\Cref{beam_bending_diagram}, the pinned support allows for rotational movements
...
\begin{itemize}
    ...
    \assume{beam_slender}{The beam is slender.}
    ...
    \assume{beam_moment_inertia_constant}{The beam has a constant bending
        moment.}
    ...
    \assume{beam_deflection}{Only relatively small deflections will be examined
        (whereby the maximum deflection is at most \texttt{SLENDER} (\(10\%\))
        of the beam's length).}

    \assume{beam_slope}{The deflection will have locally small slopes across the
        beam.}

    \assume{beam_simply_supported}{The beam is simply supported.}

    \assume{beam_loading}{The beam's loading is captured by a third-order
        polynomial of standard form, \(w_{B}(x) = a_0 + a_1x + a_2x^2 +
        a_3x^3\).}

    \assume{beam_loading_no_points}{The beam contains no point loads.}

    \assume{beam_no_axial_loading}{The beam has no loading applied axially.}

    \assume{deflection_differentiable}{The deflection function (\(y_{B}(x)\))
        belongs to the class of functions that are continuously differentiable 4
        times on \([0, L]\).}

    \assume{loading_integrable}{The loading function (\(w_{B}(x)\)) belongs to
...
% #4 equation
{
    \[\frac{1}{\rho} = \frac{\frac{d^{2}f}{da^{2}}}{(1+(\frac{df}{da})^{2})^{\frac{3}{2}}}\]
}
% #5 description
{ From elementary calculus, the curvature of a plane curve at a point
    \((a,f(a))\) of a curve may be expressed by the above
    equation~\cite{BeerJohnston1981}, where:

    \begin{itemize}
        \item \(\rho\) is the radius of curvature ($1$),
        \item \(f\) is the vertical component of the point ($1$), and
        \item \(a\) is the horizontal component of the point ($1$).
    \end{itemize}
}
% #6 Notes
{
...
[ Starting with \tref{TM:PC}:
    ...
    We may recognize that in the context of an abstract prismatic beam, the
    slope \(\frac{dy}{dx}\) is relatively ``small'' (\aref{beam_slope}).
    Similarly, its square is also small. Considering it negligible, we may drop
    it:

    \[\frac{1}{\rho} = \frac{d^{2}y}{dx^{2}}\]

    Substituting \(\frac{1}{\rho}\) from \tref{TM:PBPBA}, we obtain:
...
% #5 description
{ For a beam, the second derivative of the deflection function (i.e., the shear
  force at a point along the beam) may be expressed as a ratio of the bending
  moment ($M(x)$) to the flexural rigidity ($EI$).}
% #6 Notes
{
    None.
}
...
[ When a prismatic beam supports an applied loading, (\(w(x)\)), its elastic
    ...
    found in \tref{TM:OREC} twice~\cite{EulerBernoulliWiki}. ]
% #2 refname of theory
{TM:EBBDE}
% #3 label
...
{ The above equation describes the relationship between a beam's deflection
    ...
    (\(x\)). Notably, for a simply supported beam
    (\aref{beam_simply_supported}), the ends observe no deflection, thus:
    ...
    Additionally, as there is no resistance to rotation at the ends, the moments
    are zero at the ends (e.g., $M(0) = M(L) = 0$), and thus, as $M =
        \frac{d^{2}}{dx^2} y$:
...
\todo{I should note that the w(x) >= 0 and y(x) >= 0 is not needed because we
    can have negative deflection by negative bending. It shouldn't really affect
    the solution algorithm, I think (?).}
...
\begin{itemize}
    ...
    \item[R\refstepcounter{reqnum}\thereqnum \label{R_ValidateInputs}:] The
        provided inputs must be validated against \Cref{TblInputVar}.
