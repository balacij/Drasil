...
exprPlate = sentencePlate (concatMap sentToExp) `appendPlate` secConPlate (concatMap egetCon')
  ...
  scsSub = Constant <$> \case
    (TMs _ _ _ t)          -> goTM t
    (DDs _ _ _ des dmes _) -> go des ++ go dmes
    (GDs _ _ _ g _)        -> go g
    (IMs _ _ _ i _)        -> go i
    _ -> [],
...
sentencePlate f = appendPlate (secConPlate (f . concatMap getCon') $ f . concatMap getSec) $
  preorderFold $ purePlate {
    ...
    scsSub = Constant . f <$> \case
      (Assumptions c) -> def c
      (TMs s _ _ t) -> let r = mappend s . concatMap (\x -> def (x ^. operations) ++
                             def (x ^. defined_quant) ++ notes [x] ++
                             r (x ^. valid_context)) in r t
      (DDs s _ _ des dmes _) -> s ++ der des ++ der dmes ++ notes des ++ notes dmes
      (GDs s _ _ d _) -> def d ++ s ++ der d ++ notes d
      (IMs s _ _ d _) -> s ++ der d ++ notes d
      (Constraints s _) -> [s]
      (CorrSolnPpties _ _) -> [],
