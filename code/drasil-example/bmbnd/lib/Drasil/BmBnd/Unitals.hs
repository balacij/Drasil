module Drasil.BmBnd.Unitals where
...
inpConstrained = [ipPropGain, ipDerivGain, ipSetPt, ipStepTime, ipSimTime, opProcessVariable]
...
ipDerivGain =
  constrained'
    (dqdNoUnit derGain symKd Real)
    [physc $ UpFrom (Inc, exactDbl 0)]
    (exactDbl 1)
...
ipStepTime =
  constrained'
    (uc stepTime symTStep Real second)
...
ipSimTime =
  constrained'
    (uc simulationTime symTSim Real second)
...
opProcessVariable =
  constrained'
    (dqdNoUnit processVariable symYT (Vect Real))
    [gtZeroConstr]
    (exactDbl 1)
...
qdCtrlVarTD =
  vc
    "qdCtrlVarTD"
    (nounPhraseSent (S "Control Variable in the time domain"))
    symCT
    Real
...
qdFreqDomain =
  vc
    "qFreqDomain"
    (nounPhraseSent (S "Complex frequency-domain parameter"))
    syms
    Real
...
qdFxnTDomain =
  vc
    "qdFxnTDomain"
    (nounPhraseSent (S "Function in the time domain"))
    symFt
    Real
...
qdDampingCoeff =
  vc
    "qdDampingCoeff"
    (nounPhraseSent (S "Damping coefficient of the spring"))
    symDampingCoeff
    Real
