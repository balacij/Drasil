...
import Theory.Drasil (InstanceModel, ModelKinds(..), imNoDerivNoRefs, qwC)
import Utils.Drasil
...
transMot = imNoDerivNoRefs (ExistingModel transMotRC) 
...
rotMot = imNoDerivNoRefs (ExistingModel rotMotRC) 
...
col2D :: InstanceModel
col2D = imNoDerivNoRefs (ExistingModel col2DRC)
  [qwC time $ UpFrom (Exc, 0)
